...
export default function AssetOwnersAllocationClient({ 
  ...
  return (
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="font-semibold">{t('Allocation by Owner')}</div>
        <div className="text-xs text-slate-600">{t('Rolling anniversary allocation window (12 months)')}</div>
      </div>
      ...
      <div className="mb-4 flex items-center gap-3">
        <div className="relative flex-1 max-w-md">
          <input
            ...
            aria-label={t('Search owners by name or email')}
          />
          {query && (
            <button
              onClick={() => setQuery('')}
        ...
        <span className="text-sm text-slate-500 whitespace-nowrap">
          {t('{{visible}} / {{total}} owner{{suffix}}', {
            visible: filtered.length,
            total: owners.length,
            suffix: owners.length !== 1 ? 's' : '',
          })}
        </span>
      </div>
      ...
      <div className="space-y-6">
          ...
          <div key={o.userId} className="rounded-lg border p-4 bg-slate-50">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div className="text-xs text-slate-600">{t('Share: {{value}}%', { value: o.sharePercentage })}</div>
              </div>
            </div>
        ...
        {filtered.length === 0 && (
          <div className="p-6 text-center text-slate-500">{t('No matching owners')}</div>
        )}
      </div>
    </div>
  );
